...
def cleanup(odl_ip=None, odl_port=None):
    ...
    jetty_file = os.path.join(os.getcwd(), 'jetty.xml')
    if os.path.exists(jetty_file):
        logger.info("Removing {0}".format(jetty_file))
        os.remove(jetty_file)
    rc_file = os.path.join(os.getcwd(), 'tackerc')
    if os.path.exists(rc_file):
        logger.info("Removing {0}".format(rc_file))
        os.remove(rc_file)
    rc_file = os.path.join(os.getcwd(), 'odlrc')
    if os.path.exists(rc_file):
        logger.info("Removing {0}".format(rc_file))
        os.remove(rc_file)
    ovs_logs = os.path.join(os.getcwd(), 'ovs-logs')
    if os.path.exists(ovs_logs):
        logger.info("Removing {0}".format(ovs_logs))
        shutil.rmtree(ovs_logs)
